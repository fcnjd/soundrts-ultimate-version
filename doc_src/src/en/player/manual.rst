...
- Use the defensive mode to scout. A unit in defensive mode will flee if it encouters stronger hostile enemies (neutral units do not count). This way you can know how many enemies are in a square without sacrificing the scouting unit. The peasants are in defensive mode by default but the soldiers can be put in this mode too. Offensive and chase modes also ignore neutrals for auto-attack; use a forced attack (Ctrl+click on the unit) to fight a neutral.
...
The square is divided into smaller sub-squares (3×3 by default). Since 1.4.0.9, precision can
be adjusted from 3×3 up to 15×15:

- Ctrl+Shift+F8: increase precision
- Shift+F8: decrease precision
- Ctrl+F8: enter precision manually (e.g. ``7x7``)
- Alt+F8: announce current precision

In zoom mode, Ctrl+arrow keys force-browse through obstacles (map editing / observation).
...
Text input fields (share codes, seeds, login, chat, etc.) support Ctrl+A select all, Ctrl+C copy, Ctrl+X cut, Ctrl+V paste; pasted text is filtered to allowed characters.
...
- arrow keys: move (up=forward, down=back, left/right=strafe)
- Shift+left/right: turn
- number keys 1–9 and 0: use skills 1–10
- Alt+/: list skills and hotkeys
- Ctrl+A: auto-attack nearest enemy in sight
- combine with F8 zoom for precise movement
...
Since SoundRTS 1.4.3.4, whole maps can be generated from menu parameters (size, starts, resources, monsters, water, treasure, etc.), with seeds and share codes to reproduce the same layout.

- Local: main menu → Start a game → Random map
- Online: create game → map list → Random map

After generation, seed and share code are announced; F5/F6 still replay them from history in later menus. Full guide: `Random map guide <randommap.htm>`_.
...
Find SoundRTS.ini_ in the ``user folder`` and modify the "speed = ..." line. Use an integer number.
...
Three screens are mutually exclusive; select exactly one friendly unit:
...
.. list-table::
   ...
   * - Equipment
     - Ctrl+V
     - Weapons and armor (built-in + inventory)


Open requirements:

- Backpack: inventory must not be empty (otherwise "backpack empty" is announced).
- Equipment: at least one weapon or armor must exist (built-in or in inventory).

In backpack or equipment, the same keys apply:

- arrow keys: previous / next item
- g: read item intro (from ``style.txt`` ``intro``)
- Enter: equip weapon, wear armor, or use consumable (backpack only)
...
- pickup: default right-click or command menu when a unit has enough ``inventory_capacity``
- drop: command menu, or Delete in backpack/equipment

Giving items to NPCs (since 1.4.3.1)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

When carrying items, you can hand one to another unit:

- right-click a non-hostile unit (not a building): auto-selects "give" and walks over
- command menu: "give"
- shortcut: ``g`` (configured in ``style.txt``)

The target must have ``receive_items 1`` in rules.txt. The author can restrict which items
and which relations (ally, neutral, enemy) are accepted — see the modding guide.

Object filters
^^^^^^^^^^^^^^^

M / Shift+M: cycle target filter (friendly, enemy, all).
N / Shift+N: cycle type filter (buildings, units, elements, all).
Combine both to narrow Tab selection (e.g. friendly buildings only).

Saves and replays (since 1.4.2.6)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Each mod has up to 10 save slots and its own replays. Cancelling a game creates a memory
point; use "continue unfinished game" on the main menu.

Since 1.4.2.7:

- Shift+Enter on a save or replay in the menu: rename
- Delete: delete with confirmation
- Shift+Delete: delete immediately

Files are also in ``user/saves`` and ``user/replays``.

Dynamic alliance and treaty (since 1.4.2.3 / 1.4.2.2)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

During a game (not if alliances were fixed before start):

- F12 or Shift+F12: select alliance candidate
- F4: send alliance request
...
F12 does not work in campaigns. Trigger computers are announced as "NPC". Ctrl+Shift+F4
reports trigger computers as "NPC"; if that player has only wildlife (``is_huntable`` /
``herdable``), it says "you are animal". Selected wildlife is announced as "deer , animal",
not "neutral , NPC" (see ``../player/hunting.htm``).
