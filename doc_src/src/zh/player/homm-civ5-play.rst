...
SoundRTS 仍是即时战略：选单位、下命令、采集、造兵、交战。HoMM / Civ5 元素以实时方式接入 RMG：开局英雄通过战斗升级并解锁耗费魔法值的技能（单机可跨局保存成长）；城市占格并可购买相邻地块、分配市民与建设改良，每分钟结算产出、文化与外交点；市政科技和政策卡由城镇中心研究，本局内可切换已解锁政策。
...
.. list-table::
   ...
   * - Civ5 地图宝箱 / 奢侈品
     ...
     - ``\_append_treasure``
   * - Civ5 城市领土与地块经营
     - 购买相邻主方格、市民分配、矿场/伐木场/农场改良
     - ``rmg_systems.py`` + ``worldorders/strategic.py``
   * - Civ5 市政政策切换
     - 两槽政策；第三张替换最旧；本局内切换已解锁政策
...
RMG 战略系统（详见 `RMG 英雄与文明战略系统 <rmg-strategic-systems.htm>`_）：

- 每名玩家开局拥有 1 名 ``rmg_hero``。英雄最高 8 级，在 2 / 4 / 6 级解锁奥术箭、旋风斩、流星雨；技能消耗魔法值，魔法值会自动恢复。单机 RMG 按模组与阵营保存最高等级与经验，下一局自动恢复。
- 城镇中心、要塞和城堡均视为城市。每座城占有所在主格；可用「购买地块」向相邻格扩张（首块 20 金，之后递增）。每 60 秒结算城市与工作地块产出；可分配市民专精金/木/食/文化；派驻农民到达后用标准建造命令施工矿场、伐木场、农场。
- 城镇中心提供 ``城市规划 → 市政管理 → 外交事务`` 科技链。
- 传统、商业、外交为三张政策卡，分别强化文化、城市资源、外交点产出；同时最多启用两张，第三张会替换最早的一张；已解锁政策可在本局内免费切换。电脑按局势选用固定政策组合。
- RMG 中主动发起结盟申请消耗 20 外交点；接受、拒绝和退盟不消耗。
...
6. 模组兼容：胜利数值与战略系统由 ``rules.txt`` 的 ``rmg_*`` 参数与 ``cfg/randommap/*.txt`` 模板配置（非 Python 硬编码）。换用非标准资源名时，同步调整 ``rmg_economic_goal*``、``victory_triggers`` 与 ``grant_resources`` 中的 resource 槽。详见 ``player/RMG英雄与文明战略系统说明.htm`` 第 7 节与 ``mod/随机地图生成器说明.htm`` 8.4
...
.. list-table::
   ...
   * - 玩家：RMG 战略系统
     - [RMG 英雄与文明战略系统](rmg-strategic-systems.htm)
   * - 规则：POI 单位
