...
See also ``docs/en/burst-attack-damage-seq.md``.
...
1. **Default** — on hit
2. **``is_active 1``** — when starting an attack (active)
3. **``is_passive 1``** — when taking damage (passive), with ``trigger_condition`` (e.g.
...
``docs/en/starcraft-resources-vespene.md``. The attributes screen (V) adds **requires deposit**;
...
**``xp_threshold_growth`` curve types** (threshold index ``i`` starts at 0 for level 2, 3, …)
...
Separate from ``campaign_flag`` / ``add_inventory_item`` (story tokens, alliances). See ``docs/en/developer/campaign-hero-carryover.md``.
...
Random-map POI ``captured_barracks`` and HoMM-style play: ``docs/zh/player/英雄无敌与文明5玩法说明.md``.
...
- Terran addons: ``docs/en/starcraft-terran-addons.md``
- Zerg creep & Queen tumors: ``docs/en/starcraft-zerg-creep.md``
...
**``build_field_radius`` vs ``build_field_radius_m``**
...
Player guide: ``docs/en/starcraft-zerg-creep.md``. Mod readme: ``mods/starcraft/readme.txt``.
...
``build_field_spreads 1``. Queen **Spawn creep tumor** / tumor **Extend** — see ``docs/en/starcraft-zerg-creep.md``.
...
**``can_capture``** — ``1`` or ``0``. Default ``1``. On units with ``attack`` skills: when
...
See ``docs/en/hunting-system.md``. Summary:
...
- **Diplomacy**: a ``computer_only`` slot with **only** wildlife (``deer`` / ``sheep`` / custom ``tiger``, etc.) does not join the ``"ai"`` alliance and does not ally with players, hostile creep, or other wildlife herds; mixed slots are unchanged. See ``docs/en/hunting-system.md`` §3.1.
