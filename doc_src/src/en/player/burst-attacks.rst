...
.. list-table::
   ...
   * - Launch sounds
     - One ``launch_mdg`` / ``launch_rdg`` per shot
...
.. list-table::
   ...
   * - Stats
     - 3×2 ranged damage per cycle, 2.5 s reload, range 4
...
.. list-table::
   ...
   * - Problem
     - Cause / fix
   * - ``damage_seq`` ignored
     - Base ``mdg`` / ``rdg`` not defined, or segment sum ≠ base (manual split)
   * - Wrong interval
     - Before 1.4.3.6, interval was ignored (fixed); check game version
   * - Fractional damage + manual `(damage …)`
     - Use auto split instead
   * - More than 6 shots
     - Engine caps at 6 per attack
   * - Only one launch sound
     - Expected for non-burst units; burst units need per-shot handling (1.4.3.6+)
...
.. list-table::
   ...
   * - ``soundrts/definitions.py``
     - Parses ``damage_seq`` in rules
   * - ``soundrts/combat/damage_effects.py``
     - Schedules burst hits and launch sounds
   * - ``soundrts/combat/attack_action.py``
     - Attack prep / cooldown
   * - ``soundrts/tests/test_damage_seq_burst.py``
     - Parsing and regression tests
